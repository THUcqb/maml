...
class DataGenerator(object):
    ...
    def make_data_tensor(self, train=True):
        ...
        for _ in range(num_total_batches):
            ...
            labels_and_images = get_images(sampled_character_folders, range(
                self.num_classes), nb_samples=self.num_samples_per_class, shuffle=False)
        ...
        for i in range(self.batch_size):
            ...
            if FLAGS.datasource == 'omniglot':
                ...
                rotations = tf.multinomial(
                    tf.log([[1., 1., 1., 1.]]), self.num_classes)
            ...
            for k in range(self.num_samples_per_class):
                ...
                if FLAGS.datasource == 'omniglot':  # and FLAGS.train:
                    new_list[-1] = tf.stack([tf.reshape(tf.image.rot90(
                        tf.reshape(
                            new_list[-1][ind], [self.img_size[0], self.img_size[1], 1]),
    ...
    def generate_sinusoid_batch(self, train=True, input_idx=None):
        ...
        amp = np.random.uniform(
            self.amp_range[0], self.amp_range[1], [self.batch_size])
        phase = np.random.uniform(
            self.phase_range[0], self.phase_range[1], [self.batch_size])
        outputs = np.zeros(
        ...
        for func in range(self.batch_size):
            ...
            if input_idx is not None:
                init_inputs[:, input_idx:, 0] = np.linspace(
                    self.input_range[0], self.input_range[1], num=self.num_samples_per_class-input_idx, retstep=False)
