...
class Dyn_Model:
    ...
    def __init__(self, inputSize, outputSize, sess, learning_rate, batchsize, which_agent, x_index, y_index,
                 num_fc_layers, hidden_size, mean_x, mean_y, mean_z, std_x, std_y, std_z, tf_datatype, print_minimal):
        """Dynamics graph & trainer & evaluator (& predictor(forward sim)).
        ...
        :param y_index:
        :type y_index: int
        :param num_fc_layers: number of the MLP\'s layers
        :type num_fc_layers: int
        :param hidden_size: size of the MLP\'s intermediate layers
        ...
        self.x_ = tf.placeholder(
            tf_datatype, shape=[None, self.inputSize], name='x')  # inputs
        self.z_ = tf.placeholder(
            tf_datatype, shape=[None, self.outputSize], name='z')  # labels

        # forward pass
        self.curr_nn_output = feedforward_network(self.x_, self.outputSize,
                                                  num_fc_layers, hidden_size, tf_datatype)
        ...
        self.train_step = self.opt.minimize(self.mse_)
    ...
    def train(self, dataX, dataZ, dataX_new, dataZ_new, nEpoch, save_dir, fraction_use_new):
        ...
        for i in range(nEpoch):
            ...
            old_indeces = npr.choice(range_of_indeces, size=(
                dataX.shape[0],), replace=False)
            ...
            if batchsize_old_pts > 0:
                ...
                for batch in range(int(math.floor(nData_old / batchsize_old_pts))):
                    ...
                    if(num_new_pts == 0):
                        ...
                        new_indeces = npr.randint(
                            0, dataX_new.shape[0], (batchsize_new_pts,))
                        dataX_new_batch = dataX_new[new_indeces, :]
                        dataZ_new_batch = dataZ_new[new_indeces, :]

                    # walk through the randomly reordered "old data"
                    dataX_old_batch = dataX[old_indeces[batch *
                                                        batchsize_old_pts:(batch+1)*batchsize_old_pts], :]
                    dataZ_old_batch = dataZ[old_indeces[batch *
                                                        batchsize_old_pts:(batch+1)*batchsize_old_pts], :]

                    # combine the old and new data
                    dataX_batch = np.concatenate(
                        (dataX_old_batch, dataX_new_batch))
                    dataZ_batch = np.concatenate(
                        (dataZ_old_batch, dataZ_new_batch))
            ...
            # train completely from new set
                ...
                for batch in range(int(math.floor(num_new_pts / batchsize_new_pts))):
                    ...
                    dataX_batch = dataX_new[batch *
                                            batchsize_new_pts:(batch+1)*batchsize_new_pts, :]
        ...
        for batch in range(int(math.floor(nData_old / self.batchsize))):
            ...
            dataX_batch = dataX[batch *
                                self.batchsize:(batch+1)*self.batchsize, :]
            dataZ_batch = dataZ[batch *
                                self.batchsize:(batch+1)*self.batchsize, :]
            # one iteration of feedforward training
            loss, _ = self.sess.run([self.mse_, self.curr_nn_output], feed_dict={
                                    self.x_: dataX_batch, self.z_: dataZ_batch})
            avg_old_loss += loss
            iters_in_batch += 1
    ...
    def run_validation(self, inputs, outputs):
        ...
        :param outputs: ground-truth labels
        :returns: the average loss on the inputs
        """
        ...
        for batch in range(int(math.floor(nData / self.batchsize))):
            # Batch the training data
            dataX_batch = inputs[batch *
                                 self.batchsize:(batch+1)*self.batchsize, :]
            dataZ_batch = outputs[batch *
                                  self.batchsize:(batch+1)*self.batchsize, :]

            # one iteration of feedforward training
            loss = self.sess.run(self.mse_, feed_dict={
                self.x_: dataX_batch, self.z_: dataZ_batch})
    ...
    def do_forward_sim(self, forwardsim_x_true, forwardsim_y, many_in_parallel):
        ...
        :param forwardsim_x_true: TODO
        :type forwardsim_x_true: TODO
        :param forwardsim_y: TODO
        :type forwardsim_y: TODO
        :param many_in_parallel: TODO
        :type many_in_parallel: TODO
        """
        ...
        if(many_in_parallel):
            ...
            for timestep in range(horizon):
                ...
                states_preprocessed = np.nan_to_num(
                    np.divide((curr_states-array_meanx), array_stdx))
                actions_preprocessed = np.nan_to_num(
                    np.divide((forwardsim_y[:, timestep, :]-array_meany), array_stdy))
                inputs_list = np.concatenate(
                    (states_preprocessed, actions_preprocessed), axis=1)

                # run the N sims all at once
                model_output = self.sess.run(
                    [self.curr_nn_output], feed_dict={self.x_: inputs_list})
                state_differences = np.multiply(
                    model_output[0], array_stdz)+array_meanz
            ...
            # curr state is of dim NN input
            curr_state = np.copy(forwardsim_x_true[0])
            ...
            for curr_control in forwardsim_y:
                ...
                curr_state_preprocessed = np.nan_to_num(
                    curr_state_preprocessed/self.std_x)
                curr_control_preprocessed = curr_control - self.mean_y
                curr_control_preprocessed = np.nan_to_num(
                    curr_control_preprocessed/self.std_y)

                # why expand? dim is (batch, ...)
                inputs_preprocessed = np.expand_dims(
                    np.append(curr_state_preprocessed, curr_control_preprocessed), axis=0)

                # run through NN to get prediction
                model_output = self.sess.run([self.curr_nn_output], feed_dict={
                                             self.x_: inputs_preprocessed})
